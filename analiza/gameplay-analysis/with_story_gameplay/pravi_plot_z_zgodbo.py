...
df = pd.read_csv('final_story.csv')
...
df.to_csv('final_story.csv', index=False)

# Priprava barvne lestvice glede na frekvence do 25
...
ax.set_title('Prikaz pogostosti igranja nivojev igre z zgodbo', pad=110, fontsize=20)
ax.grid(True, axis='y')
...
sm.set_array([])
cbar = fig.colorbar(sm, ax=ax, ticks=np.arange(0, 26, 5))
